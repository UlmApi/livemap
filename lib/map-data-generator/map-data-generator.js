	...
	var gen = function(dir, cb) {
		...
		*/
		...
		var gtfs = Gtfs(dir, function(res) {
			/* callback: everything loaded & finished */
			...
			console.log("clear: " + mapData['stops'].features.length)	;
			...
			for (var i in shapes) {		// extract lon&lat
				...
				// insert the coords
				var coord = [parseFloat(shapes[i].shape_pt_lon),parseFloat(shapes[i].shape_pt_lat)];	// one pair of coords
