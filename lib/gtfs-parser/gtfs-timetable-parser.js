...
module.exports = (function(){
	...
	var calculateTripProgess = function(time, delta) {
		...
		var progressThen, startTime,endTime;
		var tripsProgress = {};
		for (var i in trips) {
			startTime = trips[i][trips[i].start].arrivalTime;
			endTime = trips[i][trips[i].end].arrivalTime;
			//console.log("tripid: "+trips[i][0].trip_id+" start: "+startTime+" end: "+endTime)
			if (startTime < time && time < endTime) {
				...
				if (gtfs.getTripById(trips[i][0].trip_id)==1) {
